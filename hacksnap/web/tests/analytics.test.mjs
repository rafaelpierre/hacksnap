...
test("browse, story, rerender, next, back: one view per route occurrence", () => {
  ...
  for (const path of ["/", "/", "/story/1", "/story/1", "/story/2", "/story/1"]) {
    ...
    if (path.startsWith("/story/")) j.emit("story_visit", {story_id: path.split("/").at(-1)}, `story:${path}`);
  }
  assert.equal(j.events.filter(e => e.name === "reader_visit").length, 4);
  assert.deepEqual(j.events.filter(e => e.name === "story_visit").map(e => e.story_id), ["1", "2", "1"]);
...
test("sharing preserves action counts but strips arbitrary content fields", () => {
  ...
  for (let i = 0; i < 2; i++) j.emit("share_open", {story_id: "1", draft: "private text", url: "mailto:private"});
  j.emit("share_destination", {story_id: "1", destination: "X"});
  assert.equal(j.events.filter(e => e.name === "share_open").length, 2);
...
test("missing browser and throwing analytics do not interrupt user actions", () => {
  assert.doesNotThrow(() => track("share_open"));
  const j = createJourney(() => { throw Error("blocked"); }, () => "id");
  assert.doesNotThrow(() => { j.route("/"); j.emit("copy_success", {copy_kind: "link"}); });
});
test("browser calls queue before GA, dispatch to GA when available, and tolerate failure", () => {
  ...
  try {
    ...
    track("story_visit", {story_id: "3"}, "story:3");
    assert.deepEqual(window.dataLayer.map(args => args[1]), ["reader_visit", "story_visit"]);
    const sent = [];
    window.gtag = (...args) => sent.push(args);
    track("copy_attempt", {copy_kind: "post"});
    assert.equal(sent[0][1], "copy_attempt");
    window.gtag = () => { throw Error("blocked"); };
    assert.doesNotThrow(() => track("copy_failure", {copy_kind: "post"}));
