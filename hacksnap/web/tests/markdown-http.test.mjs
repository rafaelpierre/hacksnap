...
test("story content and sharing are present before JavaScript executes", {skip: !base || !storyId}, async () => {
  const response = await fetch(new URL(`/story/${storyId}`, base));
  assert.equal(response.status, 200);
  const html = await response.text();
  assert.match(html, /<header class="story-header"/);
  assert.match(html, /class="story-meta"[\s\S]*?points[\s\S]*?comments on HN/);
  assert.match(html, /class="skepticism-pill /);
  assert.match(html, /<section class="tldr-section"[\s\S]*?TLDR;/);
  assert.match(html, /<section class="discussion-section"[\s\S]*?Discussion/);
  assert.match(html, /aria-label="Share: /);
});
test("synthetic stories distinguish skepticism and pending-summary states", {skip: !base || storyId !== '90000001'}, async () => {
  for (const [id, expected, absent] of [
    ['90000006', ['Low skepticism', 'TLDR;', 'Discussion'], ['Summary pending']],
    ['90000008', ['No comment evidence', 'TLDR;'], ['Summary pending']],
    ['90000009', ['Skepticism pending', 'Summary pending'], ['TLDR;']],
    ['90000010', ['High skepticism', 'Discussion'], ['Summary pending']],
  ]) {
    const response = await fetch(new URL(`/story/${id}`, base));
    assert.equal(response.status, 200);
    const html = await response.text();
    for (const value of expected) assert.ok(html.includes(value), `${id}: ${value}`);
    for (const value of absent) assert.ok(!html.includes(value), `${id}: ${value}`);
  }
});
