...
test("category metadata is constrained, projected to readers, and preserved by unclassified ingestion", async () => {
  ...
  await db.exec("BEGIN");
  try {
    const fixtureDate = new Date('2020-01-01T00:00:00Z');
    const record = {hn_id: 100, title: 'AI agent', url: 'https://example.com', date_published: fixtureDate,
      date_added: fixtureDate, author: 'test', points: 1, comment_count: 1, last_seen_run_id: current,
