...
export default function ApiDocs() {
  return <article className="detail" style={{overflowWrap: "anywhere"}}>
    ...
    <p>Returns <code>{"{stories: [...], ingestion: string | null}"}</code>. Stories follow the homepage ranking, with the same selection and a shared 10-minute data cache. The ingestion timestamp records the last successful collection. No pagination or query parameters are supported.</p>
    <h2>Get a story</h2>
    ...
    <h2>Story fields</h2>
    <p>Each story contains <code>hn_id</code> (string), <code>title</code>, <code>url</code>, <code>points</code>, <code>comment_count</code>, <code>date_added</code> (UTC timestamp), and <code>summary</code>.</p>
    ...
    <p>Errors return <code>{"{error: string}"}</code>: 400 for an invalid ID, 404 for an unknown story, and 503 when data is unavailable. Retry a 503 after 60 seconds. Cached lists may retain the last successful result during an outage. Poll the list no more frequently than every 10 minutes.</p>
