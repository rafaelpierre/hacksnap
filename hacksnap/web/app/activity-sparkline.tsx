...
import { activityChart, activityIntervals, formatRate, type ActivityObservation } from "../lib/activity-history";
...
export function ActivitySparkline({history, title, asOf}: {history: ActivityObservation[]; title: string; asOf: string}) {
  ...
  const description = `${title}. Point activity over the past 24 hours. ${detail} Each point is an interval average plotted at its ending observation. Connecting lines are visual guides, not measured intermediate rates. Scale ${formatRate(min)} to ${formatRate(max)} points/hour, scaled per story. The flat lead-in extends the first available value to the left edge; earlier activity was not measured. Use left and right arrow keys to explore.`;
  ...
  return <figure className="activity-history" aria-label={`Point activity for ${title}`}>
    ...
    {last && selected ? <>
      <div className="activity-rate">{formatRate(last.rate)}</div>
      ...
      {active !== null && <div className="sparkline-tooltip"><strong>{formatRate(selected.rate)} points/hour</strong><br />{time(selected.start)}–{time(selected.end)} UTC<br />Average between observations{selected === points[0] && <><br />Flat lead-in uses this first value; earlier activity is unknown.</>}<br />Scale: {formatRate(min)} to {formatRate(max)} pts/h</div>}
